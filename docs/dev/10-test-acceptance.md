...
| AC-03 | 语音频道状态互动 | E2E 加入语音频道，切换静音、闭麦、恢复和退出，其他成员看到变化。 |
...
| AC-E8 | 断线和语音状态恢复 | 实时测试断线超时后状态离线，语音会话释放并广播离开。 |
...
| AC-N2 | 语音状态同步不超过 3 秒 | 多客户端切换语音状态，统计状态到达时间。 |
...
| FR-16 | 语音状态 | `JoinVoiceChannel`、`LeaveVoiceChannel`、`VoiceMemberJoined`、`VoiceMemberLeft` | VoiceSession、Channel、Membership | AC-03、AC-E8 |
| FR-17 | 语音状态 | `UpdateVoiceState`、`VoiceStateChanged` | VoiceSession、PermissionOverwrite | AC-03、AC-N2 |
...
通过该数据集覆盖好友、私聊、社区、频道、权限、通知和语音状态。
...
- 文档中不把真实语音媒体流、屏幕共享、机器人平台或商业化能力作为 v1 通过条件。
