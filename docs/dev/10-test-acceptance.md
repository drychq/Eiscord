...
| 命令 | 覆盖 |
|---|---|
| `pnpm e2e:api` | 构建依赖 → 重置 `eiscord_test` 数据库 → 运行 Jest + Supertest + Socket.IO E2E，覆盖 AC-01 至 AC-06、AC-E1 至 AC-E8 和 AC-N4。 |
| `pnpm e2e:web` | 构建依赖 → 生成测试音频 → 重置测试库 → 执行演示 seed → 启动 API/Web 并运行 Playwright 浏览器 E2E。 |
| `pnpm e2e:voice` | Playwright 双 context 模拟两名用户先后加入 `voice-room`，注入合成音轨，验证 mediasoup 协商完成、后加入成员可消费已有 Producer、对端 RTP 帧可解码、静音/闭麦生效；可选附加 Worker kill 用例验证自动重新加入。 |
| `pnpm e2e` | 依次运行 `e2e:api` 和 `e2e:web`。 |
...
| 编号 | 场景 | 验收方式 |
|---|---|---|
| AC-01 | 用户注册、登录并发起私聊 | E2E 创建两个用户，通过搜索 username 发起好友申请并建立好友关系，发送私聊消息并确认双方可见；API 层保留 UUID 好友申请兼容性测试。 |
...
| AC-E9 | 密码重置安全 | API 测试覆盖：未注册邮箱、注册邮箱、错误 OTP、过期 OTP、5 次错误锁定均返回符合反枚举约定的响应；成功路径写入新密码并吊销该用户全部活跃 AuthSession；MailHog 抓取邮件内容验证 OTP 不写入审计 metadata 与日志。 |
...
| FR-03 | 账号与身份 | `POST /auth/forgot-password`、`POST /auth/reset-password`、`MailerService.sendPasswordResetCode` | User（`password_reset_*` 字段）、AuthSession、AuditLog | AC-01、AC-E9 |
...
| FR-06 | 社交与私聊 | `SearchUsers`、`CreateFriendRequest`、`AcceptFriendRequest`、`NotificationCreated` | User、Friendship、Notification、AuditLog | AC-01、AC-E3 |
...
- M6 通过时至少运行 `pnpm e2e:api`、`pnpm e2e:web`；非功能验收运行 `pnpm perf:k6` 并记录 P95 指标。
