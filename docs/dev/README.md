...
| 语音（含真实音频媒体流） | FR-16、FR-17 |
| 通知与未读 | FR-18 |
| 角色与权限 | FR-19、FR-20 |

P1 包含密码找回、多端细节适配、离线推送、社区搜索增强、审计查询界面和更多消息管理能力。P2 不进入 v1，包括屏幕共享、开放接口市场、商业化订阅、广告投放、复杂内容审核、独立运营后台，以及服务端录音、混音和语音转写。
...
| 媒体 SFU | mediasoup（TypeScript Worker，自托管，仅音频 Opus） |
| ICE/TURN | coturn（短 TTL HMAC 凭证） |
| 本地环境 | Docker Compose |
...
- v1 语音频道承载 Opus 音频流（mediasoup SFU）；服务端不进行录音、混音或语音转写，入站 Producer 强制 `kind === 'audio'`，禁止视频与屏幕共享轨。
