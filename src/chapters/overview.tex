...
\begin{itemize}
  ...
  \item \textbf{性能可达}：文本消息端到端可见时间不超过 1 秒（NFR-01）、未读与通知 2 秒内同步、语音状态 3 秒内同步（NFR-02）、媒体协商 P95 不超过 3 秒（AC-N2）、单房间 ≥ 4 人音频丢包率 ≤ 3\%、jitter ≤ 30 ms（AC-N6）。
...
\begin{enumerate}
  ...
  \item mediasoup AudioLevelObserver 上报当前最强声源，服务端节流（≤ 2 次/秒）后广播 \texttt{VoiceActiveSpeaker}。
