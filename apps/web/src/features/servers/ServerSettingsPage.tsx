...
function ChannelsTab({
  ...
  serverId: string;
  channels: { channel_id: string; name: string; type: string; topic: string | null; sort_order: number }[];
  canManage: boolean;
}) {
  const [modalOpen, setModalOpen] = useState(false);
...
function ChannelFormModal({
  ...
  initial: { name: string; type: string; topic: string | null } | null;
  onClose: () => void;
  onSave: (data: { name?: string; type?: string; topic?: string }) => void;
  saving: boolean;
}) {
  const [name, setName] = useState(initial?.name ?? '');
